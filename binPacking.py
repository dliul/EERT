...
"""
    FFD_split can only work for one type of cluster
    and it currently only deals with one cluster
"""
...
def FFD_split_test(taskset, clusters):    
    tasks = sorted(taskset, key=lambda x:x.utilization, reverse=True)
    clusters.reset_map()
    mapped = []
    for p in clusters.get_allProcessors():
...
##        map_flag = False # use this flag to check whether the task is mapped
        for t in tasks:
            if not t in mapped and p.get_totalUtil() + t.get_utilization() <= 1:
                p.map_one_task(t)
                mapped.append(t)
##                if sa.QPA(p.get_alltasks()) == -1:
##                    break
...
##                # mappedTasks = p.get_alltasks()
                if t in mapped:
                    continue
                split_done = False
                leftCapacity = 0.9999 - p.get_totalUtil()
                wcet_1 = math.floor(leftCapacity * t.get_period())
                # Once wcet_1 is not equal to 0, the algorithm tries to reduce wcet
                while wcet_1 != 0:
                    spt_1 = task("%s-1" %t.get_id(), wcet_1, t.get_period(), wcet_1,t.get_coefficient())
                    # Check the total utilization less than 1
                    ...
                    if p.get_totalUtil() + spt_1.get_utilization() <= 1 and not p.get_split_1():                            
                        ...
                        if p.get_split_1():
                            break
                if split_done:
                    break # complete the mapping for the current task, move to the next processor                  
    return clusters
